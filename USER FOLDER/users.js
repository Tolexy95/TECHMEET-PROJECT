...
//   try {
//     const response = await fetch("https://techmeetappwebapi.onrender.com/api/Users");
...
async function getUsers() {
  try {
    ...
    data.forEach((user) => {
      ...
      const userProfileLink = document.createElement("a");
      userProfileLink.href = `/PROFILE FOLDER/profile.html?username=${userName}`; // Update the link to include the username as a query parameter
      userProfileLink.textContent = "Go to user profile";
      userProfileLink.classList.add("otherUserProfile");
      userContainer.appendChild(userProfileLink);

      const chatLink = document.createElement("a");
      chatLink.href = `/chatBox Folder/chat.html?currentUser=${currentUser}&receiver=${user.userName}`; // Update the link to include the sender and receiver usernames as query parameters
      chatLink.textContent = "Chat with user";
      chatLink.classList.add("chatWithUser");
      userContainer.appendChild(chatLink);
